 ...
 * @author Elf
 */
public class GorillasGUI extends JFrame {
	public ElfGorillas ctx;
	Image helmImg = null;
	Image capeImg = null;
	Image neckImg = null;
	Image ammoImg = null;
	 ...
	 * @throws IOException
	 */
	public GorillasGUI(ElfGorillas main) throws IOException {
		this.ctx = main;
		initComponents();
	}
	...
	public void initComponents() throws MalformedURLException {
		...
		auguryToggle.setToolTipText("<html>\n TEXT \n</html>");
		...
		auguryToggle.setIcon(new ImageIcon(GorillasGUI.class.getResource("/elfGorillas/resources/redSquare.png"))); //
		auguryToggle.setSelectedIcon(
				new ImageIcon(GorillasGUI.class.getResource("/elfGorillas/resources/greenSquare.png"))); //
		getContentPane().add(auguryToggle);
		auguryToggle.setBounds(30, 120, 50, 50);

		rigourToggle.setToolTipText("<html>\n TEXT \n</html>");
		auguryToggle.setBorder(BorderFactory.createBevelBorder(BevelBorder.RAISED));
		...
		rigourToggle.setIcon(new ImageIcon(GorillasGUI.class.getResource("/elfGorillas/resources/redSquare.png"))); //
		rigourToggle.setSelectedIcon(
				new ImageIcon(GorillasGUI.class.getResource("/elfGorillas/resources/greenSquare.png"))); //
		rigourToggle.addActionListener(e -> {
			System.out.println(rigourToggle.isSelected());

		});
		getContentPane().add(rigourToggle);
		rigourToggle.setBounds(30, 180, 50, 50);

		vengToggle.setToolTipText("<html>\n TEXT \n</html>");
		...
		vengToggle.setIcon(new ImageIcon(GorillasGUI.class.getResource("/elfGorillas/resources/redSquare.png"))); //
		vengToggle.setSelectedIcon(
				new ImageIcon(GorillasGUI.class.getResource("/elfGorillas/resources/greenSquare.png"))); //
		getContentPane().add(vengToggle);
		vengToggle.setBounds(30, 240, 50, 50);

		desertToggle.setFont(new java.awt.Font("Arial", 0, 10)); // NOI18N
		desertToggle.setToolTipText("<html>\n TEXT \n</html>");
		desertToggle.setBorder(BorderFactory.createBevelBorder(BevelBorder.RAISED));
		desertToggle.setBorderPainted(false);
		desertToggle.setEnabled(false);
		desertToggle.setContentAreaFilled(false);
		desertToggle.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));
		desertToggle.setIcon(new ImageIcon(GorillasGUI.class.getResource("/elfGorillas/resources/redSquare.png"))); //
		desertToggle.setSelectedIcon(
				new ImageIcon(GorillasGUI.class.getResource("/elfGorillas/resources/greenSquare.png"))); //
		...
		loadWornButton.addActionListener(e -> {
			...
			try {
				for (int i = 0; i < mainGearIds.length; i++) {
					if (mainGearIds[i] == 22109) {
						URL urlString = new URL("https://everythingrs.com/img/gameitem/10499.png");
						urlList.add(urlString);
					} else {
						URL urlString = new URL("https://everythingrs.com/img/gameitem/" + mainGearIds[i] + ".png");
						urlList.add(urlString);
		...
		backgroundLabel.setIcon(new ImageIcon(GorillasGUI.class.getResource("/elfGorillas/resources/GorillasGui.png"))); // NOI18N
